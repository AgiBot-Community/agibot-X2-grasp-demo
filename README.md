...
真机抓取在夹紧、抬升和高位后撤后，保持夹爪闭合，先垂直抬到回撤高度，再水平撤到
起始末端位置上方，垂直下降到起始位置，最后恢复任务开始时读取的精确关节姿态。
回撤高度取实际规划后撤终点高度与“起始末端高度 + initial_upward”的较大值。
四个阶段各使用 `duration`，分别发布 `return_raising`、`returning_high`、
`return_lowering`、`returning`；全部完成后才发布 `completed`、播放语音并返回成功。
回位笛卡尔路径在运动前完成 IK 规划，并采样检查关节插值路径偏差，容差复用
`high_retract_position_tolerance`（默认 5 mm）。规划失败不回退为直接关节回位。
最终恢复原始关节姿态使用关节插值，不受笛卡尔路径约束；整个流程不包含持物碰撞检测。
